...
(instance debugRm of Feature
	...
	(method (handleEvent event &tmp temp0 temp1 [temp2 4] [temp6 200] temp206 temp207 temp208 temp209 temp210 temp211 temp212 temp213 temp214 temp215 temp216 temp217)
		;(if (event claimed:) ;move under evKEYBOARD to allow claimed event passthrough to mouse
		;	(return)
		;)
		(switch (event type:)
			(evKEYBOARD
				(if (event claimed:) (return)) ;reject claimed keyboard events
				(event claimed: 1)
				(switch (event message:)
					...
					(KEY_ALT_i
						(gGame setCursor: 999) ;use arrow cursor
						(dInvD doit:)
