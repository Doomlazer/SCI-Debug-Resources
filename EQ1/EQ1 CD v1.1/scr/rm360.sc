	...
	(method (changeState newState)
		(switch (= state (= state (= state (= state newState))))
			...
			(1
				...
				(= ticks (= ticks (= ticks (= ticks theTicks))))
			)
			(2
	...
	(method (changeState newState)
		(switch (= state (= state (= state (= state newState))))
			...
			(1
				...
				(= ticks (= ticks (= ticks (= ticks theTicks_2))))
			)
			(2
...
(instance dolphinNet of Script
	...
	(method (changeState newState)
		(switch (= state (= state (= state (= state newState))))
			...
			(12
				(= ticks (= ticks (= ticks (= ticks (Random 10 90)))))
			)
			(13
				(if
...
(instance cycleStuff of PalCycle
	...
	(method (atInt param1)
		(= intLo
			(= intLo (= intLo (= intLo [theIntLo param1])))
		)
		(= intHi
			(= intHi (= intHi (= intHi [theIntLo (+ param1 1)])))
		)
	)
)
...
(instance dolphinOsc of Osc
	...
	(method (doit &tmp oscNextCel)
		(if
			...
			(= cycleDir
				(= cycleDir (= cycleDir (= cycleDir (- cycleDir))))
			)
			(self cycleDone:)
		else
