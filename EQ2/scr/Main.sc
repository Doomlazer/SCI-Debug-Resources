...
(class Rain of Game
	...
	(method (handleEvent event)
		(if (== (pEvent type?) evMOUSEBUTTON)
			((ScriptID 98) handleEvent: pEvent) ;send mouse events to debug
		)
		(if (event claimed:)
			(return 1)
		)
		(if (== (event type:) evKEYBOARD)
			((ScriptID 98) handleEvent: pEvent) ;send keyboard events to debug
	...
	(method (points param1 param2)
		...
		(if param1
			...
			;(if param2
			(if (&exists param2) ;cursor freeze bug fix
				(SetFlag param2)
			)
			...
			((& temp1 evMOUSEBUTTON)
				(cond
					...
					((& (event modifiers:) emCTRL)
						(if (gUser canControl:)
							(if (not (== (event modifiers?) 12)) ;ignore CTRL-ALT-Click for debug command
								(self swapCurIcon:)
							)
						)
						(event claimed: 1)
